$(document).ready(function() {
	...
	var students = [];
	var knowledgenames = [];
	if(!args.uuid || !args.openid || !args.classid || args.classid == ""){
		$.showTip({
			title: "错误",
			...
			if(!type || type == "exam"){
				api.getReport({
					...
					success: function(res) {
						...
						examineid = res.report.examineid;
						var examTitle = res.report.title;
						if(examTitle && examTitle != ""){
							document.title = examTitle;
						}
						$(".paperTitle").text(examTitle);
		...
		initDom: function(report) {
			...
			if(students && students.length > 0){
				...
				for(var m = 0; m < students.length; m++){
					var stuRank_list = $("<li></li>").addClass("stuRank_list").attr("studentid",students[m].stid);
		...
		if(analyTo && analyTo.length > 0){
			...
			api.getKnowledges({
				...
				success: function(jsonData) {
					knowledgenames = jsonData.rows;
					var knowledgename = "名字获取中...";
					...
					for(var p = 0; p < analyTo.length; p++) {
						//查找知识点name
						for(var i = 0, len = knowledgenames.length; i < len; i++) {
							if(knowledgenames[i].id == analyTo[p].knowledgeid) {
								knowledgename = knowledgenames[i].title;
							}
						}
	...
	//学生报表
	$(".studentRank").on("click",".stuRank_list",function(){
		var studentid = $(this).attr("studentid");
		window.location.href = "./reportstudent.html?uuid=" + uuid + "&openid=" + openid + "&uploadid=" + uploadid + "&examineid=" + examineid + "&studentid=" + studentid + "&classifyid=" + args.classifyid;
	});
