...
//                }
        ...
        close: function (editor, $img) {
            ...
            $img.css({
                ...
                if ( $(this).parent().remove().hasClass("edui-image-upload-item") ) {
					...
					$.get(window.UMEDITOR_BACKEND + 'richeditor.remove', { editorid: editor.id, url: img.attr('src') });
                }
        ...
        callback: function (editor, $w, url, state) {
            ...
            if (state == "SUCCESS") {
                //显示图片计数+1
                Upload.showCount++;
                var $img = $("<img src='" + (url.indexOf('://') == -1 ? (editor.options.imagePath + url) : url) + "' class='edui-image-pic' />"),
    ...
    var Upload = {
        ...
        init: function (editor, $w) {
			...
			me.toggleMask("正在获取已上传图片列表....");

			$.get(window.UMEDITOR_BACKEND + 'richeditor.imagelist', { editorid: me.editor.id }, function(response) {
				if (response.length) {
					var files = response.split('\n');
					for(var i = 0; i < files.length; ++ i) {
						var url = files[i].trim();
						if (url.length) {
							if (input) {
							...
							Base.callback(me.editor, me.dialog, files[i], 'SUCCESS');
						}
					}
				}
        ...
        submit: function (callback) {
            ...
            var me = this,
                input = $( '<input style="filter: alpha(opacity=0);" class="edui-image-file" type="file" hidefocus="" name="upfile" accept="image/gif,image/jpeg,image/png,image/jpg">'),
                input = input[0];
        ...
        initEvt: function () {
            ...
            $(".edui-image-searchAdd", me.dialog).on("click", function () {
                ...
                if (url) {
                    ...
                    $("<img src='" + (url.indexOf('://') == -1 ? (editor.options.imagePath + url) : url) + "' class='edui-image-pic' />").on("load", function () {
