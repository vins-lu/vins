$(document).ready(function() {
    ...
    if((!gradeid || gradeid == "") || (!classid || classid == "")){
        $.showTip({
            content: "请选择年级和班级!",
            success: function(){
                window.location.href = "../teacher_selectclass.html?openid=" + openid + "&uuid=" + uuid + "&schoolid=" + schoolid;
            }
        });
        return;
    }
    if(!classifyid || classifyid == ""){
        $.showTip({
            content: "请选择一个科目!",
            success: function(){
                window.location.href = "./selectsubject.html?openid=" + openid + "&uuid=" + uuid + "&schoolid=" + schoolid + "&classid=" + classid + "&gradeid=" + gradeid;
            }
        });
        return;
    }
