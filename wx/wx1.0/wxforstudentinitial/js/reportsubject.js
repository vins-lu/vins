$(document).ready(function() {
	...
	function initDom(report) {
		...
		if(subject && !$.isEmptyObject(subject)){
			...
			for(var item in subject){
				...
				if(subject[item]){
					...
					totalnums = totalnums + parseInt(itemDetail.length);
			...
			if(!isNaN(totalnums) && totalnums > 5){
				$(".questionAnalysis").find(".showMore").removeClass("hidden");
			}
		...
		if(data && !$.isEmptyObject(data)){
			...
			if(!isNaN(total_errkd) && total_errkd > 2){
				$(".knowledgeAnalysis").find(".showMore").removeClass("hidden");
			}
		}else{
	...
	$.smartScroll($(".analyse"),".analyse_con");
	...
	//展示全部错误知识点信息
	$(".knowledgeAnalysis").on("click",".showMore",function(){
		$(this).parent().find(".detail").addClass("showAll");
		$(this).remove();
	});
	//收起全部题目信息
