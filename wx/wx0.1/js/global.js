...
//var baseUrl = "http://api.lm.com/";

function PageLoader() {}
pageLoader = new PageLoader();
pageLoader.url = baseUrl;
pageLoader.codeurl = baseUrl + "validcodes.show?";
pageLoader.platform = 'wx';
pageLoaderautoReloadPage = 0;
pageLoader.frame = {
	width: 0,
	height: 0,
};

function genQRCode(pageurl, action, uuid, params) {
	var gets = '';
	...
	var pagename = pageurl.split('/').join('%2F');

	return ['https://open.weixin.qq.com/sns/getexpappinfo?appid=wx6745d3d25c905b5a&path=', pagename, '.html%3Faction%3D', action, '%26uuid%3D', uuid, gets, '#wechat-redirect'].join('');
}
				...
				if(response.ok || response.rows != null) {
					...
					if(!err_callback || !err_callback(response)) {
						if(response.lang) {
							...
							$.showTip("发生网络错误，错误信息为: " + errCode(response.errcode.toString()) + "，请拨打客服电话 400-640-8688 以解决此问题", function() {
								if(response.errcode == 2 || pageLoader.autoReloadPage){
									(parent || top).location.reload();
								}

								$("body").emptyState({
									type:"exception",
									content:errCode(response.errcode.toString()) + "，请拨打客服电话 400-640-8688 以解决此问题",
									replace:true,
									style:{
										position: "absolute",
										top: "38.2%",
...
function showPageLoader() {
	...
	var modal = $(document.createElement("div")).addClass("modalConver").addClass("loadingmodal").append(img);
	$("body").addClass("modal-open").append(modal);
}

function removePageLoader() {
	$($("body > .loadingmodal")[0]).remove();
...
function getUserType(sign) {
	...
	array["grademain"] = "年级组长";
	array["teacher"] = "教师";
	array["superteacher"] = "副校长";
	array["classmain"] = "班主任";
	array["region"] = "大区经理";
	array["offteacher"] = "内部教师";
	array["service"] = "客服";
	array["admin"] = "系统管理员";
	array["super"] = "超管";
	return array[sign];
...
// 触摸反馈
$.fn.touchFeedback = function(){
	return $(this).each(function() {
		this.onclick = function(e){
			var _this = $(this);
			_this.addClass("touchFeedback");
			...
			rippleDiv.css({
			    ...
			    background: $(this).attr('data-ripple-background') ? $(this).attr('data-ripple-background') : "#36bdb6"
			}).appendTo($(this));

			if($(".ripple").length <= 0){
				_this.removeClass("touchFeedback");
			}
...
function NumberToChinese(num){
	...
	for(var i = 0; i < len; i++){
		if(i == len - 1){
			res += arr1[parseInt(arr[i]) - 1];
		}else{
			if(parseInt(arr[i]) == 0){
				res += "零";
			}else{
				res += arr1[parseInt(arr[i]) - 1] + arr2[len - i - 1];
			}
		}
	}
	return res;
