$(document).ready(function() {
	...
	function initDom(report) {
		...
		if(subject && !$.isEmptyObject(subject)){
			for(var item in subject){
				...
				var getScore = $("<span></span>").addClass("getScore");
				getScore.progress({
					value: scorePercent,
					labeltext: "得分率" + scorePercent + "%",
					labeltextStyle: {
						fontSize: "0.8em",
						color: "#333",
					},
            ...
            legend: {
                ...
                data: loseQustionSet.slice(0,4).map(function(item,index){
	            	if(index == 3){
	                	...
	                	return "第" + item.no + "题(" + getSubjectName(item.type) + ")"
	                }
	            }),
	            selectedMode:false,
            },
            series : [
                {
                    ...
                    data: loseQustionSet.slice(0,4).map(function(item,index){
                    	if(index == 3){
	                    	...
	                    	return {
	                    		value: item.lose, 
	                    		name: "第" + item.no + "题(" + getSubjectName(item.type) + ")"
	                    	}
	                    }
                    }),
                    itemStyle: {
	...
	function initHistoryCharts(examHistory) {
		...
		chart_classRank.on('click', function (params) {
		    var seriesIndex = params.seriesIndex;
		    var dataIndex = params.dataIndex;
	        chart_score.dispatchAction({
	            type: 'showTip',
	            seriesIndex: 0,
	            dataIndex: dataIndex
	        });
		...
		chart_score.on('click', function (params) {
		    var seriesIndex = params.seriesIndex;
		    var dataIndex = params.dataIndex;
	        chart_classRank.dispatchAction({
	            type: 'showTip',
	            seriesIndex: 0,
	            dataIndex: dataIndex
	        });
